...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (settingsRes.ok) {
      ...
      storedPassword = doc.fields?.password?.stringValue ?? null
      storedAdminUids = doc.fields?.adminUids?.arrayValue?.values
    ...
    // Already admin — nothing to write
    if (storedAdminUids.includes(callerUid)) {
      return res.status(200).json({ adminUids: storedAdminUids })
    }
    ...
    console.log(`[promote-to-admin] ${callerUid} promoted`)
    res.status(200).json({ adminUids: newUids })
  } catch (err) {
    const msg = err instanceof Error ? err.message : 'Internal error'
    console.error('[promote-to-admin]', msg)
