...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── 3. Read current adminUids from Firestore ───────────────────────────
    ...
    // ── 4. Authorise: caller must already be in adminUids ─────────────────
    if (!storedAdminUids.includes(callerUid)) {
      return res.status(403).json({ error: 'Forbidden' })
    }

    // ── 5. Apply the new adminUids array ──────────────────────────────────
    ...
    console.error('[set-admin-uids] error:', msg)
