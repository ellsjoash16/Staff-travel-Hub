...
export function LoginScreen() {
  ...
  async function handleForgotPassword() {
    ...
    try {
      ...
      const code = (err as { code?: string }).code
      if (code === 'auth/too-many-requests') { toast.error('Too many attempts — please wait a few minutes and try again') }
      else { toast.error('Could not send reset email — check the address and try again') }
    } finally { setBusy(false) }
  }
  ...
  async function handleSignUp() {
    ...
    if (!/@(dialaflight|dafconcierge|lotusgroup)\.co\.uk$/i.test(email.trim())) { toast.error('You must use your @dialaflight.co.uk, @dafconcierge.co.uk or @lotusgroup.co.uk email to sign up'); return }
  ...
  async function handleMicrosoft() {
    ...
    try {
      ...
      if (!/@(dialaflight|dafconcierge|lotusgroup)\.co\.uk$/i.test(cred.user.email ?? '')) {
        await cred.user.delete()
        toast.error('You must use your @dialaflight.co.uk, @dafconcierge.co.uk or @lotusgroup.co.uk account')
      }
    } catch (err: unknown) {
      const code = (err as { code?: string }).code
  ...
  return (
    <div className="min-h-dvh flex flex-col lg:flex-row lg:bg-background">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        ...
        <div className="absolute inset-0" style={{ backgroundImage: 'linear-gradient(to right, hsl(var(--background) / 0) 80%, hsl(var(--background)) 96%, hsl(var(--background)) 100%)' }} />
